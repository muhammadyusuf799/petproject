        ...
        .then((response) => {
            if (response.ok) {
                ...
                return response.json().then((data) => {
                    throw new Error(
                        data.detail ||
                            "Failed to create the post. Check your input."
                    );
                });
            }
        })
    ...
    fetch(url, {
        ...
        .then((data) => {
            ...
            if (data.pagination.total_pages) {
                upperPageNum.textContent = `Page ${data.pagination.current_page} of ${data.pagination.total_pages}`;
                upperPaginationDiv.appendChild(upperPageNum);
            } else {
                upperPageNum.textContent = "No posts found!";
                upperPaginationDiv.appendChild(upperPageNum);
            }
            ...
            data.posts.forEach((post) => {
                ...
                postDescription.textContent = `${post.description.slice(
                    0,
                    180
            ...
            if (data.pagination.total_pages) {
                lowerPageNum.textContent = `Page ${data.pagination.current_page} of ${data.pagination.total_pages}`;
